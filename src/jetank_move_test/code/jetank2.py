...
class JetankController(Node):
    def __init__(self):
        ...
        self.traj_pub = self.create_publisher(
            JointTrajectory, 
            '/jetank2/arm_controller/joint_trajectory', 
            qos_profile
        )
        ...
        for i in range(1, 11):
            self.jenga_pubs[i] = {
                'attach': self.create_publisher(Empty, f'/jetank2/jenga{i}/attach', qos_profile),
                'detach': self.create_publisher(Empty, f'/jetank2/jenga{i}/detach', qos_profile)
            }
        ...
        self.MAGNET_FRAME = 'jetank2/MAGNETIC_BAR_1' 
...
def main():
    ...
    try:
        ...
        # 환경 변수로 드롭 위치( x y z roll )를 받을 수 있음.
        drop_override = os.environ.get("JETANK2_DROP")
        ...
        while True:
            try:
                ...
                if drop_override:
                    try:
                        dx, dy, dz, dr = [float(v) for v in drop_override.replace(",", " ").split()]
                        drop_x, drop_y, drop_z, drop_roll = dx, dy, dz, dr
                    except Exception:
                        print(f"[Warn] JETANK2_DROP 파싱 실패: '{drop_override}', 기본값 사용")
                ...
                robot.move_to_xyz(0, -150, 50, phi=phi, roll=drop_roll, move_time=mv_time)
                #robot.move_to_xyz(drop_x, drop_y, hover_z, phi=phi, roll=drop_roll, move_time=mv_time)
                time.sleep(mv_time + 5.0)

                # 내려놓기
                robot.move_to_xyz(drop_x, drop_y, drop_z, phi=phi, roll=drop_roll, move_time=mv_time)
                time.sleep(mv_time + 5.0)
                robot.control_magnet("OFF")
                time.sleep(10.0)
                robot.move_to_xyz(drop_x, drop_y, 0, phi=phi, roll=drop_roll, move_time=mv_time)
                time.sleep(mv_time + 5.0)
